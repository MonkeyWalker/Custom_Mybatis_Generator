...
public class ProviderUpdateByExampleWithoutBLOBsMethodGenerator extends
    ...
    public void addClassElements(TopLevelClass topLevelClass) {
        ...
        method.addBodyLine(String.format("%s conditions = (%s) parameter.get(\"conditions\");", //$NON-NLS-1$
                example.getShortName(), example.getShortName()));

        if (useLegacyBuilder) {
            method.addBodyLine("applyWhere(conditions, true);"); //$NON-NLS-1$
            method.addBodyLine("return SQL();"); //$NON-NLS-1$
        } else {
            method.addBodyLine("applyWhere(sql, conditions, true);"); //$NON-NLS-1$
